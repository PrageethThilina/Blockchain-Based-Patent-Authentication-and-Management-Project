...
class UsptoDashboard extends Component {
    ...
    if (networkData) {
      ...
      if (patentCount.toString() === '0') {
        ...
        for (var i = 1; i <= patentCount; i++) {
          ...
          if (this.state.account === "0x11a6dB8497a849abd3c0b73643E8Fd4f103D9fd5") {
            if (approved_check_patent_claim.USPTO_Approval === "Approved") {
              this.setState({
  ...
  render() {
    ...
    return (
      <div>
        <UsptoNavbar account={this.state.account} />
        <div id="content" style={{ marginTop: '60px' }}>
          <div id="USPTOPendingPatents">
            {this.state.loading
